-- OpenCode MySQL 持久化：staging 表结构对齐生产
-- 集群 27736 / 分片 mysql-47627-20250714 / 库 ai_code
--
-- 背景：staging 的 oc_* 表由应用 MysqlMigration.migrate 自动创建（oc_schema_migration.binary_version
-- 为 1.18.1-mysql.20260715），带 13 个 FOREIGN KEY 与 ON DELETE CASCADE。生产 KDB 禁止外键，
-- 级联删除已改由 MysqlSessionRepository.remove / removeMessage 显式处理。两边结构不一致会让孤儿数据
-- 问题在 staging 被 cascade 掩盖、只在生产暴露，故必须对齐。
--
-- 为何用 RENAME 而非 DROP：KDB 审核对「存在外键的表」禁止 DROP（提示需人工工单），但 RENAME TABLE
-- 可通过自动审核。RENAME 后旧表连同外键一起改名留存，等价于备份，且无需 DBA 介入。
--
-- 数据影响：staging 全部 oc_* 数据不再被应用读取（约 oc_event 33006 / oc_part 7117 / oc_message 1817 /
-- oc_session 264 行），均为测试产生，无保留价值。旧表以 _legacy_20260731 后缀原地留存，可随时查阅或回滚。
-- 确认无需回滚后，另提工单 DROP 这批 _legacy_20260731 表以释放空间。
--
-- ⚠️ 重建后 binary_version 为 manual-ddl，应用重启不会再自动 migrate（check 只校验 version 是否齐全）。
-- 这是预期行为：避免应用把外键重新加回来。此后新增 migration 需在 staging 与生产都手工提工单。
--
-- 执行前置：确认 staging 的 OpenCode 实例已停写。

USE `ai_code`;

-- 阶段 1：旧表改名留存。RENAME 会连带外键一起迁移到新表名，不影响后续建表。
RENAME TABLE oc_part TO oc_part_legacy_20260731;

RENAME TABLE oc_message TO oc_message_legacy_20260731;

RENAME TABLE oc_session_message TO oc_session_message_legacy_20260731;

RENAME TABLE oc_session_input TO oc_session_input_legacy_20260731;

RENAME TABLE oc_todo TO oc_todo_legacy_20260731;

RENAME TABLE oc_session_context_epoch TO oc_session_context_epoch_legacy_20260731;

RENAME TABLE oc_permission_request TO oc_permission_request_legacy_20260731;

RENAME TABLE oc_question_request TO oc_question_request_legacy_20260731;

RENAME TABLE oc_run TO oc_run_legacy_20260731;

RENAME TABLE oc_event TO oc_event_legacy_20260731;

RENAME TABLE oc_event_sequence TO oc_event_sequence_legacy_20260731;

RENAME TABLE oc_session TO oc_session_legacy_20260731;

RENAME TABLE oc_project TO oc_project_legacy_20260731;

RENAME TABLE oc_user_fence TO oc_user_fence_legacy_20260731;

RENAME TABLE oc_request_dedup TO oc_request_dedup_legacy_20260731;

RENAME TABLE oc_schema_migration TO oc_schema_migration_legacy_20260731;

-- 阶段 2：按生产结构重建。以下与生产工单 909498 的 SQL 逐字一致（仅去掉其 USE 语句）。
